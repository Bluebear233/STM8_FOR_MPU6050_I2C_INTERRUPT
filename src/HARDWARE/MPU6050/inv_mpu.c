...
#define MPU_IIC_Init() I2C_Interrupt_Confing(400000)
#define i2c_write(a,b,c,d)   I2C_Multiple_Write_With_Block(a,b,d,c)
#define i2c_read(a,b,c,d)   I2C_Multiple_Read_With_Block(a,b,d,c)
...
data[0] = (long) ((35 + ((raw - (float) st.hw->temp_offset) / st.hw->temp_sens))
		* 65536L);
return 0;
}
...
accel[0] =
		(long) (((long long) accel[0] << 16) / test.accel_sens / packet_count);
accel[1] =
		(long) (((long long) accel[1] << 16) / test.accel_sens / packet_count);
accel[2] =
		(long) (((long long) accel[2] << 16) / test.accel_sens / packet_count);
/* Don't remove gravity! */
if (accel[2] > 0L)
	accel[2] -= 65536L;
else
